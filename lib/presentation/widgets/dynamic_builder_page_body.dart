...
class _DynamicBuilderPageBodyState extends State<DynamicBuilderPageBody> {
  ...
  Future<void> _createOrEditRow(
    ...
    for (final TableColumnEntity c in columns) {
      ...
      } else if (c.type != TableColumnType.formula) {
      ...
      builder: (BuildContext context) {
        return StatefulBuilder(
          builder: (BuildContext context, StateSetter setModalState) {
            Map<String, dynamic> modalWorkingValuesForFormulas() {
              ...
              for (final TableColumnEntity col in columns) {
                ...
                m[col.id] = currentDraftValue(col);
              }
              return m;
            }

            String formulaColumnDisplayText(TableColumnEntity col) {
              final TableRowEntity synthetic = TableRowEntity(
                id: existing?.id ?? '00000000-0000-0000-0000-000000000001',
                tableId: schema.id,
                values: modalWorkingValuesForFormulas(),
              );
              ...
              final dynamic v = resolved[col.id];
              if (v == null) {
                return '';
              }
              return v.toString();
    ...
    for (final TableColumnEntity col in columns) {
      ...
      } else if (col.type != TableColumnType.formula) {
