...
final List<TableColumnType> _columnTypeOptions = TableColumnType.values
    .where(
      (TableColumnType type) =>
          type != TableColumnType.autoGenerated,
    )
...
class EditTableScreen extends GetView<EditTableController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Obx(() {
        ...
        final int dynamicMiddleSteps = isCrudStandard ? 1 : 0;
        final int totalSteps = 5 + dynamicMiddleSteps;
            ...
            steps: <Step>[
              ...
              Step(
                ...
                isActive:
                    ...
                    (controller.isSummaryTable
                        ? 4
                        : (controller.isCrudStandardTable ? 5 : 4)),
                state: StepState.indexed,
  ...
  Widget _stepDesignLayout(ThemeData theme, BuildContext context) {
    return Obx(() {
      final bool isSummary = controller.isSummaryTable;
      if (isSummary) {
        return _summaryDesignLocked(theme);
      }
  ...
  Widget _summaryDesignLocked(ThemeData theme) {
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        Text(
          'Summary tables are defined at creation time. The source configuration '
          'and column structure cannot be changed here; you can still edit the table name, '
          'page, and description, and other runtime options.',
          style: theme.textTheme.bodySmall?.copyWith(
            color: theme.colorScheme.onSurfaceVariant,
          ),
        ),
        if (s.summaryConfig != null) ...<Widget>[
          const SizedBox(height: 16),
          Text('Configured summary', style: theme.textTheme.titleSmall),
          const SizedBox(height: 8),
          for (final SummaryColumnConfig c
              in s.summaryConfig!.columns) ...<Widget>[
            ListTile(
              contentPadding: EdgeInsets.zero,
              title: Text(c.name),
              subtitle: Text(
                '${c.groupBy ? "Group by · " : ""}${c.valueMode.name}',
              ),
            ),
          ],
        ],
    ...
    return Obx(() {
      ...
      return Column(
        ...
        children: <Widget>[
          ...
          const SizedBox(height: 12),
          ...controller.columns.map(
            (EditColumnDraft column) => _editColumnCard(theme, column, context),
          ),
  ...
  Widget _summaryAggregationPreviewForEdit(ThemeData theme) {
    List<String> headers = controller.columns
        .map((EditColumnDraft c) {
  ...
  Widget _inventoryCard(ThemeData theme, BuildContext context) {
    return Obx(() {
      if (controller.isSummaryTable) {
        return const SizedBox.shrink();
      }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              const SizedBox(height: 6),
              Text(
                'When enabled, saving a new row in this table subtracts the line quantity '
                'from a numeric column on another table.',
                style: theme.textTheme.bodySmall?.copyWith(
                  color: theme.colorScheme.onSurfaceVariant,
                ),
              ),
              ...
              SwitchListTile(
                ...
                onChanged: (bool v) {
                  controller.inventoryDeductionEnabled.value = v;
                },
              ),
              if (controller.inventoryDeductionEnabled.value) ...<Widget>[
                if (controller.inventoryStockTableOptions.isEmpty)
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: controller.inventoryStockTableOptions
                        .map(
                          (TableSchemaEntity s) => DropdownMenuItem<String>(
                            value: s.id,
                            child: Text(s.name),
                          ),
                        )
                    ...
                    onChanged: (String? id) {
                      ...
                      controller.invStockQuantityColumnId.value = null;
                    },
                  ),
                  const SizedBox(height: 12),
                  if (controller.inventoryStockTableColumns.isEmpty)
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: controller.inventoryStockTableColumns
                          ...
                          .toList(growable: false),
                      onChanged: (String? id) {
                        controller.invStockMatchColumnId.value = id;
                      },
                    ),
                    const SizedBox(height: 12),
                    DropdownButtonFormField<String>(
                      value: controller.invStockQuantityColumnFieldValue,
                      decoration: const InputDecoration(
                        labelText: 'Stock quantity column (to subtract)',
                      ),
                      items: controller.inventoryStockNumericColumns
                          .map(
                            (TableColumnEntity c) => DropdownMenuItem<String>(
                              value: c.id,
                              child: Text('${c.name} (${c.type.storageValue})'),
                            ),
                          )
                          .toList(growable: false),
                      onChanged: (String? id) {
                        controller.invStockQuantityColumnId.value = id;
                      },
                    ),
                  ],
                  const SizedBox(height: 12),
                  ...
                  const SizedBox(height: 8),
                  if (controller.inventoryLineProductCandidates.isEmpty)
                    Text(
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: controller.inventoryLineProductCandidates
                          .map(
                            (EditColumnDraft c) => DropdownMenuItem<String>(
                              value: c.id,
                              child: Text(
                          ...
                          .toList(growable: false),
                      onChanged: (String? id) {
                        controller.invLineProductColumnId.value = id;
                      },
                    ),
                  const SizedBox(height: 12),
                  if (controller.inventoryLineQuantityCandidates.isEmpty)
                    Text(
                      'Add a number column for quantity.',
                      style: theme.textTheme.bodySmall?.copyWith(
                        color: theme.colorScheme.error,
                      ),
                    )
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: controller.inventoryLineQuantityCandidates
                          .map(
                            (EditColumnDraft c) => DropdownMenuItem<String>(
                              value: c.id,
                              child: Text(
                          ...
                          .toList(growable: false),
                      onChanged: (String? id) {
                        controller.invLineQuantityColumnId.value = id;
                      },
                    ),
                ],
              ],
                          ...
                          if (column.formulaInputMode.value ==
                            ...
                            return FormulaTextEditorField(
                              ...
                              currentColumnNames: controller.columns
                                  ...
                                  .toList(growable: false),
                              errorText:
                                  controller.formulaFieldErrors[column.id],
                              onChanged: () {
